...
from saq.service import SERVICE_STATUS_RUNNING, SERVICE_STATUS_STOPPED
from saq.test import ACEBasicTestCase, log_count, wait_for_log_count
...
class TestCase(ACEBasicTestCase):
    def setUp(self, *args, **kwargs):
        ...
        for key in get_config()['service_network_semaphore'].keys():
            if key.startswith('semaphore_'):
                del get_config()['service_network_semaphore'][key]
    ...
    def test_acquire_release_defined_network_semaphore(self):
        get_config()['service_network_semaphore']['semaphore_test'] = '3'
